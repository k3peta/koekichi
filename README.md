...
KoeKichi(声吉)は、Whisper によるローカル完結の音声入力ユーティリティです。グローバルホットキーで録音し、文字起こし結果をアクティブなアプリケーションのカーソル位置へ挿入します。

## プラットフォーム別リポジトリ

このリポジトリは **macOS 版の正本** です。Windows 版は UI、キーフック、インストール方法、貼り付け処理が macOS と大きく異なるため、現時点では別リポジトリで管理しています。

| 対象 | リポジトリ | 位置づけ |
|---|---|---|
| macOS | <https://github.com/k3peta/koekichi> | 本リポジトリ。Apple Silicon Mac 向けの安定版 |
| Windows | <https://github.com/k3peta/koekichi-win> | Windows 10/11 向け。HUD、タスクトレイ、Windows 用インストーラを別実装 |

辞書形式、URL 展開、誤入力ガード、句読点整形、履歴形式などは、将来的に共通化しやすいように揃えていきます。ただし、日常利用の安定性を優先し、当面はプラットフォームごとに配布・検証します。
...
| 項目 | macOS |
|---|---|
| OS | macOS 13+ / **Apple Silicon (arm64) のみ** |
| Python | 3.12(uv が自動取得) |
| GPU | Apple Silicon GPU (MLX) |
| 既定モデル | mlx-community/whisper-large-v3-turbo |

Intel Mac は非対応です(起動時にエラー終了します)。

Windows 版を使う場合は、専用リポジトリ <https://github.com/k3peta/koekichi-win> を参照してください。
...
初回起動時は Whisper モデルのダウンロードが行われます(Mac 既定モデルは約 1.6GB)。
...
Windows 版のダウンロード、インストール、アンインストール、既知の制限は <https://github.com/k3peta/koekichi-win> を参照してください。

Windows 版は faster-whisper を中心にした CPU 動作を既定とし、HUD、タスクトレイ、インストーラ、キーフックを Windows 専用に実装しています。macOS 版と思想は同じですが、現時点では別アプリとして検証しています。
...
Windows 版の手動テスト、配布物、インストール手順は、Windows 専用リポジトリ <https://github.com/k3peta/koekichi-win> で管理します。macOS 版の変更を Windows 版へ反映する場合は、辞書・整形・誤入力ガードなどの共有しやすい部分だけを選んで移植し、UI や OS 権限まわりは各プラットフォームで個別に確認します。
