...
- GUI の設定画面・辞書編集画面(設定/辞書は JSON ファイルを直接編集。トレイメニューから開ける)。
...
- トレイメニュー「辞書を再読み込み」で再読込。録音開始時にファイル mtime が変わっていれば自動再読込。
...
- メニュー:
  ...
  - `設定ファイルを開く` / `辞書を開く` / `辞書を再読み込み`(OS 既定アプリで開く: mac `open`, win `os.startfile`)
